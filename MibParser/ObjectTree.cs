...
namespace Lextm.SharpSnmpLib.Mib
{
    ...
    internal sealed class ObjectTree : IObjectTree
    {
        ...
        public Definition Find(IList<uint> numerical)
        {
            if (numerical == null)
            {
...
// ReSharper restore LoopCanBePartlyConvertedToQuery
        ...
        {
            foreach (KeyValuePair<string, ITypeAssignment> pair in ((MibModule)module).Types)
            {
        ...
        {
            var def = Search(v.Id.ToNumerical()).Definition as Definition;
